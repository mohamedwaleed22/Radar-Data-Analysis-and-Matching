...
def match(path1, path2):
    ...
    mints = int(time_diffrence.split(':')[0].lstrip('0'))
    secnds = int(time_diffrence.split(':')[1].lstrip('0'))
    ...
    for file in xls_files:
        ...
        df_list = pd.read_excel(file, header=None, skiprows=2)
        big_df = df_list.rename(columns={2: 'Date', 3: 'HeightMSL', 4: 'Speed', 5: 'Dir', 6: 'Lat', 7: 'Lon'})
        big_df = big_df.drop([0, 1], axis='columns')
        big_df = big_df.drop(big_df[(big_df['Lat'] == '//////') & (big_df['Lon'] == '//////')].index, axis=0)
        big_df['Date'] = big_df['Date'].astype(str).str[:19]
        k = big_df.shape[0]
        range1 = big_df['Date'].iloc[0]
        range11 = datetime.strptime(range1, '%Y-%m-%d %H:%M:%S')
        range2 = big_df['Date'].iloc[k - 1]
        range22 = datetime.strptime(range2, '%Y-%m-%d %H:%M:%S')
        ...
        for file2 in csv_files:
            range3 = datetime.strptime(file2.split('Output02foh')[1].split('.')[0], '%y%m%d%H%M%S')

            if range11 <= range3 <= range22:
                csvfiles2.append(file2)

                df_list2 = (pd.read_csv(file3) for file3 in csvfiles2)
                big_df2 = pd.concat(df_list2, ignore_index=True)
                big_df2 = big_df2.dropna(axis=1)


                # loop through .xls files DataFrame
                for z in range(0, len(big_df)):
                                ...
                                for j in range(0, big_df2.shape[1]):
                                    ...
                                    if time_df1 <= abs_diff_sec:
                                        ...
                                        if (lat_diff <= lat_lon_diff) and (long_diff <= lat_lon_diff) and \
                                            ...
                                            row2 = pd.Series(
                                                {"Lat": Lat, "Long": Long, "Alt": alt, "Date": dateFinalWR1,
                                                 "SpeedUP": SpeedUp,
...
# example input: C:/Users/dell g3/PycharmProjects/Z_files/matrouh2022-10
# example input: C:/Users/dell g3/PycharmProjects/Z_files/MUH_NCweatherB,C

# matching files function
match(path1=str(input("please include / within .xls path1: ")),
      path2=str(input("please include / within .csv path2: ")))
